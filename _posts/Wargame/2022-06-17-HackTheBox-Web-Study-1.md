...
그리고 접속한 페이지의 이름을 출력시켜주는데 Flask/Jinja2에서 SSTI 취약점이 발생하는 것을 알고 있기 때문에 {% raw %}{{7*7}}{% endraw %}를 페이지의 이름으로 작성하여 리퀘스트를 보내면 다음과 같은 응답을 받을 수 있다.
...
popen 함수 오브젝트는 \__subclasses_\_ 에서 414번째 인덱스에 존재했고 이를 통해 리눅스 명령어를 호출하면 된다.
